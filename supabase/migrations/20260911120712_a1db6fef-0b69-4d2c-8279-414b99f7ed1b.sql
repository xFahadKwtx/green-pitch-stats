-- Additive refresh-ahead support.
--
-- h2_get_or_claim_ahead is the single implementation. It behaves EXACTLY like
-- the previous h2_get_or_claim, except that a payload whose remaining freshness
-- is at or below p_min_fresh_ms (max 120000 ms) is not short-circuited as
-- 'fresh': the caller may claim this feed's own lease and refresh ahead of
-- expiry. Nothing else changes -- the hard TTL, per-feed leases, global
-- budgets, cooldown, backoff, deadlines and lease duration are untouched, and
-- a claim never clears the still-valid payload.
--
-- h2_get_or_claim keeps its exact signature and behaviour by delegating with a
-- zero window, so existing callers are unaffected.
...
begin
  ...
  if p_min_fresh_ms is null or p_min_fresh_ms < 0 or p_min_fresh_ms > 120000 then
...
grant execute on function public.h2_get_or_claim_ahead(text, integer, integer) to service_role;
...
set search_path = public
as $function$
  select public.h2_get_or_claim_ahead(p_cache_key, p_schema_version, 0);
$function$;
