-- Scheduled warming support (additive).

create extension if not exists pg_net with schema extensions;
create extension if not exists pg_cron;

-- Randomly generated warming token, stored ONLY in the encrypted vault.
do $$
begin
  if not exists (select 1 from vault.secrets where name = 'h2_warm_token') then
    perform vault.create_secret(
      replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', ''),
      'h2_warm_token',
      'H2 internal feed-warming endpoint token'
    );
  end if;
end $$;

-- Server-side authorization check. Returns jsonb so PostgREST yields an object.
create or replace function public.h2_verify_warm_token(p_token text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select decrypted_secret into s from vault.decrypted_secrets where name = 'h2_warm_token' limit 1;
  if s is null then
    return jsonb_build_object('authorized', false);
  end if;
  -- Fixed-length digest comparison; the secret itself is never returned.
  return jsonb_build_object('authorized', md5(p_token) = md5(s));
end;
$function$;

revoke all on function public.h2_verify_warm_token(text) from public;
revoke all on function public.h2_verify_warm_token(text) from anon;
revoke all on function public.h2_verify_warm_token(text) from authenticated;
grant execute on function public.h2_verify_warm_token(text) to service_role;

-- Widen the refresh-ahead bound for players/store ONLY (360000 ms), so a ~10min
-- scheduled warm can bridge to the next tick. Everything else is unchanged:
-- hard TTL, per-feed leases, budgets, cooldown, backoff, pacing, deadlines.
...
begin
  ...
  v_max_window := case when p_cache_key ~ ':(players|store)$' then 360000 else 120000 end;
